...
export default function ITInventory({ orgId }: { orgId: string }) {
  const { user } = useAuth();
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${isOff ? "bg-destructive/10" : "bg-primary/10"}`}>
                    <Server className={`w-5 h-5 ${isOff ? "text-destructive" : "text-primary"}`} />
                  </div>
                  <div className="min-w-0">
                    <h3 className="font-semibold text-sm truncate">{tagName}</h3>
                    <p className="text-xs text-muted-foreground">{processCount} process(es)</p>
                  </div>
                </div>
              ...
              <div className="space-y-1 max-h-40 overflow-auto">
                {details.map((d) => (
                  ...
                  >
                    ...
                    {d.steps.length > 0 && (
                      <span className="ml-auto text-[10px] text-muted-foreground shrink-0">{d.steps.length} steps</span>
                    )}
                  </button>
                ))}
                {details.length === 0 && <p className="text-xs text-muted-foreground italic">No linked processes</p>}
              </div>
            </div>
          );
        })}
      ...
      {sortedTags.length === 0 && (
        <div className="text-center py-16 text-muted-foreground">
          <Server className="w-14 h-14 mx-auto mb-4 opacity-30" />
